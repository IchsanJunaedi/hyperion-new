    ...
    <div className={`rounded-xl border bg-[#202020] p-4 space-y-3 ${expiring ? "border-red-500/40" : "border-[#2D2D2D]"}`}>
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <span className={`text-[10px] font-semibold uppercase ${STATUS_BADGE[contract.status] ?? ""}`}>
            {contract.status}
          </span>
            ...
            <div className="flex items-center justify-between">
              ...
              {isPaidThisMonth ? (
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-xs text-green-400 font-medium">Sudah dibayar</span>
                </div>
              ) : (
                <div className="flex items-center gap-1">
                  <Clock className="h-3.5 w-3.5 text-red-400" />
                  <span className="text-xs text-red-400 font-medium">Belum dibayar</span>
                </div>
              )}
            </div>
            {!isPaidThisMonth && (
              <button
                ...
                disabled={paying}
                className="w-full inline-flex h-8 items-center justify-center gap-1.5 rounded-md border border-green-500/30 bg-green-500/10 px-3 text-xs font-semibold text-green-400 hover:bg-green-500/20 hover:border-green-500/50 disabled:opacity-50 cursor-pointer transition-colors"
              >

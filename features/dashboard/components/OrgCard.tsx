  ...
  return (
    <div className="rounded-xl border border-white/10 bg-zinc-900/40 p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        {editing ? (
          <div className="flex items-center gap-1 flex-1 mr-2">
            ...
            <button onClick={() => { setEditing(false); setEditName(org.name); }} className="p-1 text-white/40 hover:bg-white/10 rounded">
        ...
        {!editing && (
          <div className="flex gap-1">
            <button onClick={() => setEditing(true)} className="p-1.5 text-white/40 hover:bg-white/10 hover:text-ui-text rounded-md" title="Edit nama">
              <Pencil className="h-3.5 w-3.5" />
            </button>
            <button onClick={handleDelete} disabled={pending} className="p-1.5 text-white/40 hover:bg-white/10 hover:text-red-400 rounded-md" title="Hapus tim">
      ...
      <div className="space-y-1.5">
        ...
        {divisions.length === 0 ? (
          <p className="text-xs text-white/30 italic">Belum ada divisi</p>
        ) : (
          <div className="flex flex-wrap gap-1.5">
            {divisions.map((div) => (
              <span key={div.id} className="inline-flex items-center gap-1 rounded-full bg-white/5 px-2.5 py-1 text-xs text-white/70">
                {div.name}
                <button
        ...
        {showAddDiv ? (
          <div className="flex items-center gap-2 mt-2">
            ...
            <button onClick={() => setShowAddDiv(false)} className="p-1 text-white/40 hover:text-ui-text">
              <X className="h-3.5 w-3.5" />
            </button>
          </div>
        ) : (

    ...
    return (
      <div className="flex min-h-[320px] flex-col items-center justify-center gap-4 rounded-2xl border border-dashed border-ui-border bg-ui-surface p-10 text-center">
        <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-ui-elevated">
          <FlaskConical className="h-6 w-6 text-ui-text-muted" />
        </div>
        <div>
          <p className="text-sm font-semibold text-ui-text">Belum ada data draft</p>
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {ROLES.filter((r) => data.byRole[r]?.length).map((role) => (
          <div
            key={role}
            className="rounded-2xl border border-ui-border bg-ui-surface overflow-hidden"
          >
            <div className="border-b border-ui-border px-4 py-3">
              <p className="text-xs font-semibold text-ui-text">{ROLE_LABELS[role]}</p>
            </div>
            <div className="divide-y divide-ui-border">
...
function HeroRow({
  ...
  return (
    ...
    >
      <span className="w-4 shrink-0 text-center text-[10px] font-bold text-ui-text-muted">
        {rank}
      </span>
      <div
        className={cn(
      ...
      <span className="flex-1 truncate text-xs font-medium text-ui-text">
        {hero.hero_name}
      </span>
      <span className="w-8 shrink-0 text-right text-[11px] text-ui-text">{hero.picks}×</span>
      <span
        className={cn(

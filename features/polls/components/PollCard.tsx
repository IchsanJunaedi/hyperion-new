...
const PollCard = ({ poll, orgSlug, canManage, userId }: PollCardProps) => {
  ...
  return (
    <div className="rounded-xl border border-ui-border bg-ui-surface p-4">
        ...
        {options.map((option, i) => {
            ...
            return (
              <div key={i} className="relative">
                <div
                  ...
                  style={{ width: `${pct}%` }}
                />
                <div className={`relative flex items-center justify-between rounded-md px-3 py-1.5 text-xs ${isMyVote ? "text-purple-400 font-medium" : "text-ui-text-2"}`}>
          ...
          return (
            <button
              ...
              className="flex w-full items-center rounded-md border border-ui-border px-3 py-1.5 text-xs text-ui-text-2 transition hover:border-purple-400/30 hover:bg-ui-hover hover:text-ui-text disabled:opacity-50 cursor-pointer"
            >
              {option}
            </button>
          );
      ...
      <div className="mt-3 flex items-center justify-between text-[10px] text-ui-text-muted">

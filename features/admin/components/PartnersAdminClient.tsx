  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-xl font-black uppercase tracking-tight text-ui-text">Partners</h1>
        <button
            ...
            {editing?.id === partner.id ? (
              ...
              <div className="flex items-center gap-4 border border-ui-border bg-ui-bg p-4">
                ...
                <div className="min-w-0 flex-1">
                  <p className="font-bold text-ui-text">{partner.name}</p>
                  <p className="text-xs text-ui-text-2">{partner.is_active ? "Aktif" : "Nonaktif"} · order: {partner.sort_order}</p>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => setEditing(partner)} className="cursor-pointer rounded p-2 text-ui-text-2 transition hover:bg-ui-hover hover:text-ui-text-dim">
                    <Pencil className="h-4 w-4" />
                  </button>
                  <button onClick={() => setDeleting(partner)} className="cursor-pointer rounded p-2 text-ui-text-2 transition hover:bg-ui-hover hover:text-red-400">

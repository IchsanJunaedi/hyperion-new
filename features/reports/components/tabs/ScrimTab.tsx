  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {([
          { label: "Total",  value: scrims.total,  sub: "scrim selesai", cls: "text-ui-text" },
          ...
          { label: "Seri",   value: scrims.draws,  sub: "draw",          cls: "text-ui-text-2" },
        ] as const).map((c) => (
          <div key={c.label} className="rounded-xl border border-ui-border bg-ui-surface p-4 space-y-1">
            ...
            <p className="text-xs text-ui-text-2">{c.sub}</p>
          </div>
        ))}
      </div>
      ...
      {scrims.total > 0 && (
        <div className="rounded-xl border border-ui-border bg-ui-surface p-5 space-y-2">
          <div className="flex items-center justify-between text-xs">
            <span className="text-ui-text-2">Win Rate</span>
            <span className={`font-bold ${scrims.winRate >= 50 ? "text-emerald-400" : "text-rose-400"}`}>
              {scrims.winRate}%
            </span>
          </div>
          <div className="flex h-2 rounded-full overflow-hidden bg-ui-elevated">
      ...
      {scrims.byDivision.length > 1 && (
        <div className="rounded-xl border border-ui-border bg-ui-surface overflow-hidden">
          <div className="px-5 py-3 border-b border-ui-border">
            <p className="text-xs font-semibold uppercase tracking-wider text-ui-text-2">Per Divisi</p>
          </div>
          <div className="divide-y divide-ui-border">
            {scrims.byDivision.map((div) => (
              ...
              >
                <span className="text-sm text-ui-text truncate">{div.divisionName}</span>
                <span className="text-xs text-ui-text-2 w-8 text-center">{div.total}</span>
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-1.5 rounded-full bg-ui-elevated overflow-hidden">
                    <div
            ...
            {scrims.list.map((s) => (
              <div key={s.id} className="flex items-center gap-3 px-5 py-2.5">
                {s.isWin === true
                  ...
                  : <MinusCircle className="h-4 w-4 shrink-0 text-ui-text-muted" />}
                <span className="text-sm text-ui-text truncate flex-1">vs {s.opponentName}</span>
                <span className="text-[11px] text-ui-text-muted shrink-0">
                  {FORMAT_LABELS[s.format] ?? s.format.toUpperCase()}
                </span>
                {s.divisionName && (
                  <span className="hidden sm:inline text-[10px] bg-ui-elevated text-ui-text-2 px-2 py-0.5 rounded-full shrink-0">
                    {s.divisionName}
                  </span>
                )}
                <span className="text-[11px] text-ui-text-muted shrink-0">
      ...
      {scrims.total === 0 && (
        <p className="text-sm text-ui-text-muted text-center py-8">Tidak ada scrim di bulan ini.</p>
      )}
    </div>
  );
};
